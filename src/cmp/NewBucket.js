...
const NewBucket = ({ onRefresh }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await axiosClient.post(`/buckets/create`, {
        s3System: currentS3System,
        bucketName,
      });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-fluid">
          <div className="field mt-5">
            <span className="p-float-label">
              ...
              <label
                htmlFor="bucketName"
                className={classNames({ "p-error": errors.bucketName })}
              >
                Name*
              </label>
            </span>
          ...
          <div className="flex gap-5 items-center justify-end mt-5">
            <button
              className="h-12 bg-green-500 border border-gray-100 w-24 text-white rounded"
              type="submit"
            >
              Submit
            </button>
            <button
              className="h-12 bg-gray-800 border border-gray-100 w-24 text-white rounded"
              type="cancel"
